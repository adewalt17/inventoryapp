<?php

  #The URL root is the AWS meta data service URL where metadata
  # requests regarding the running instance can be made
  $urlRoot="http://169.254.169.254/latest/meta-data/";

  # Get the instance ID from meta-data and print to the screen
  $instance = file_get_contents($urlRoot . 'instance-id');
  $network = file_get_contents($urlRoot . 'network');
  $ipaddress = file_get_contents($urlRoot . 'local-ipv4');
  

  # Availability Zone
  $az = file_get_contents($urlRoot . 'placement/availability-zone');

?>

<center>This page was generated by instance <b><?= $instance ?></b> in Availability Zone <b><?= $az ?></b>. The IP address of the instance is <b><?= $ipaddress ?></b> and sits on <b><?= $network ?></b> network.</center>